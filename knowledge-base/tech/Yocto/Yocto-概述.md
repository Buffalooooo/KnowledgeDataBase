...
- [[#核心概念详解\|核心概念详解]]
  - [[#Poky（参考构建系统）\|Poky]]
  - [[#BitBake（任务执行引擎）\|BitBake]]
  - [[#Recipe（配方）\|Recipe]]
  - [[#Layer（层）\|Layer]]
  - [[#Machine（机器）\|Machine]]
  - [[#Distribution（发行版）\|Distribution]]
  - [[#Image（镜像）\|Image]]
- [[#Poky 目录结构\|Poky 目录结构]]
- [[#核心概念关系图\|核心概念关系图]]
- [[#与 Buildroot 对比\|与 Buildroot 对比]]
- [[#版本命名规则\|版本命名规则]]
- [[#要点\|要点]]
- [[#参考\|参考]]
...
两者都是嵌入式 Linux 构建工具，但设计哲学完全不同。

### 一句话区别

> **Yocto** = 给你一套配方和工具，让你**定制**每个细节 → 适合复杂产品
> **Buildroot** = 给你一个菜单，让你**选择**要什么 → 适合简单设备

### 嵌入式 Linux 构建方案演变

| 时代 | 方案 | 做法 |
|------|------|------|
| 第一代（2000s） | **手工打造** | 自己下载内核/busybox/工具链，写 Makefile 串起来。每个项目从头来 |
| 第二代（2001+） | **Buildroot** | 把手工流程写成自动化 Makefile 脚本，选包 → 自动下载编译 → 出镜像 |
| 第三代（2010+） | **Yocto** | 引入 BitBake 引擎 + Recipe 描述 + Layer 机制，解决项目复用和增量构建 |

> `make menuconfig` 是 Linux **内核**的配置界面（Kconfig），Buildroot **借用**了这个界面作为自己的包选择界面。两者不是同一个东西。

### 详细对比

| 维度       | Yocto                 | Buildroot   |
| -------- | --------------------- | ----------- |
| 学习曲线     | 陡峭（几周上手）              | 平缓（几小时上手）   |
| 构建时间     | 首次 2-6 小时             | 首次 10-30 分钟 |
| 包数量      | 5000+（OE-Core 核心库）    | 2000+       |
| 灵活性      | 极高，Layer 机制可覆盖任何细节    | 中等，菜单式配置    |
| 二进制缓存    | sstate-cache 增量构建     | 每次从头编译      |
| 企业使用     | 广泛（NXP、TI、Intel 官方支持） | 较少，多用于原型    |
| Layer 机制 | **有**（核心优势）           | 无（整个系统一个配置） |
| SDK 生成   | 内置 `populate_sdk`     | 需手动配置       |
| 适用场景     | 量产产品、需要长期维护           | 原型验证、个人项目   |

### 类比

- **Yocto** = 盖房子找**建筑师**出全套图纸 + 施工队按图施工。前期慢，但每块砖都按你要求来
- **Buildroot** = 去**宜家**买一套样板房。快，但只能在现有设计里选

### 选型建议

| 场景 | 推荐 |
|------|------|
| 自己玩的个人项目 | Buildroot — 1 天搞定 |
| 量产 10 万台设备，需长期 OTA 维护 | **Yocto** — 前期投入值得 |
| 外包项目，交付一次不再管 | Buildroot — 快速出活 |
| 需提供 SDK 给客户开发 | **Yocto** — 自带 SDK 生成 |
| 多硬件变体（Wi-Fi/4G/蓝牙版） | **Yocto** — Layer 机制天然支持 |

---

## 版本命名规则

Yocto 每 6 个月发布一个版本（4 月 / 10 月），每 2 年发布一个 LTS（长期支持版本）。版本代号按字母表顺序排列，全部使用**动物名**。

### 近期版本

| 版本代号 | 动物 | 发布时间 | 类型 |
|---------|------|---------|------|
| `kirkstone` | 岩石（地名派生） | 2022.04 | LTS（上一个） |
| `mickledore` | 无峰驼 | 2023.10 | 常规 |
| `nanbield` | 一种小羊 | 2024.03 | 常规 |
| `scarthgap` | 一种海鸟 | 2024.10 | **LTS（当前）** |
| `styhead` | 某品种猪 | 2025.05 | 最新常规 |
...
# 切换到 LTS 版本（推荐学习用）
git checkout scarthgap-4.0.20

# 切换到最新版
git checkout styhead
```

> 学习阶段推荐使用 **`scarthgap`**（当前 LTS），社区支持稳定，文档匹配度最高。
